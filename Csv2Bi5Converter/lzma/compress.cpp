...
static SRes elzmaProgress(void *p, UInt64 inSize, UInt64 outSize)
{
    ...
    if (ps->progressCallback) {
        ps->progressCallback(ps->progressContext, (size_t)inSize,
                             (size_t)ps->uncompressedSize);
    }
    return SZ_OK;
}
                   ...
                   void * progressContext)
{
    ...
    {
        unsigned char * hdr =
            (unsigned char *)hand->allocStruct.Alloc(&(hand->allocStruct),
...
elzma_get_dict_size(unsigned long long size)
{
    ...
    return 1 << ((((1 << i) - (unsigned int)size) > ((unsigned int)size - (1 << (i-1)))) ? i-1 : i);
}
